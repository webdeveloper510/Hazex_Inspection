...
jQuery(document).ready(function () {
    ...
    theme_conrols.on('click', '.customizer-repeater-new-field', function () {
        if(jQuery("#industryup_service_limit").val()==3)
        {
           jQuery(".customizer_industryupservice_upgrade_section").show();
            return false;
        }
        else
        {
           var total_services_sections=parseInt(jQuery("#industryup_service_limit").val())+1;
           jQuery("#industryup_service_limit").val(total_services_sections)
           if(jQuery("#industryup_service_limit").val()>3)
        {
           jQuery(".customizer_industryupservice_upgrade_section").hide();
        }
        }
    ...
    theme_conrols.on('click', '.social-repeater-general-control-remove-field', function () {
        ...
        var total_service= parseInt(jQuery("#industryup_service_limit").val())-1;
       jQuery("#industryup_service_limit").val(total_service);
