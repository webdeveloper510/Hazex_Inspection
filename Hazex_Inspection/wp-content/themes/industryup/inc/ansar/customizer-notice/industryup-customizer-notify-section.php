...
class Industryup_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'industryup-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'industryup_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'industryup_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		foreach ( $industryup_customizer_notify_recommended_actions as $key => $industryup_lite_customizer_notify_recommended_action ) {
			if ( $industryup_customizer_notify_show_recommended_actions[ $industryup_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $industryup_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$industryup_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $industryup_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $industryup_customizer_notify_recommended_action['plugin_slug'] );
				$industryup_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $industryup_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$industryup_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$industryup_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$industryup_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$industryup_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$industryup_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$industryup_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$industryup_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$industryup_lite_customizer_notify_recommended_action['button_label'] = $industryup_deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $industryup_lite_customizer_notify_recommended_action;
		}// End foreach().
		...
		foreach ( $industryup_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $industryup_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $industryup_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			if ( ! empty( $plugin_opt['description'] ) ) {
				...
				$ti_customizer_notify_recommended_plugin['description'] = $info->industryup_description;
			}
		...
		$json['total_actions']       = count( $industryup_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="industryup-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="industryup-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="industryup-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="industryup-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no industryup-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="industryup-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="industryup-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no industryup-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
